...
parser.add_argument('--PCA_SIZE', type=int, default = 512) # must be less than batch size
...
def CreateFeatureFiles(ipca = None):
    ...
    if ipca is None:
        ipca = joblib.load(os.path.join(args.OUTPUT_PATH, "Style_Features_PCA.pkl")) 
    ...
    for i in range(imageCount):
        ...
        try:
            ...
            vgg19_Style_Dict[imageFile.replace(args.WIKIART_PATH, '')] = ipca.transform([style_out])[0]
        except:
        ...
        if (i % args.BATCH_SIZE == 0 or i == imageCount-1):
            ...
            vgg19_Style_Features.to_csv(os.path.join(args.OUTPUT_PATH, "vgg19_Style_Features_Batch_" + str(i) + ".csv"))
            vgg19_Style_Dict = {}
...
def CreateFeaturePCA():
    ...
    joblib.dump(ipca, os.path.join(args.OUTPUT_PATH, "Style_Features_PCA.pkl"))
    return ipca

ipca = CreateFeaturePCA()
CreateFeatureFiles(ipca)
fh.MergeFeatureFiles(args.OUTPUT_PATH, "vgg19_Style_Features")
